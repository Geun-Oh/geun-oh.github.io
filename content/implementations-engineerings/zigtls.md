...
TLS 1.3 in RFC 8446 is naturally implemented as a role-partitioned partial DFA.  
The core rule is:

- `δ: Q × Σ ⇀ Q` (partial transition function)
- if `δ(state, event)` is undefined, fail immediately as `IllegalTransition`

This implementation maps wire `HandshakeType` values to semantic events (`classifyEvent`), then applies `machine.onEvent(event)`.

Client-side transition diagram (`δ_c`):

```mermaid
stateDiagram-v2
  [*] --> wait_server_hello
  wait_server_hello --> wait_encrypted_extensions: server_hello
  wait_server_hello --> wait_server_hello: hello_retry_request
  wait_encrypted_extensions --> wait_server_certificate: encrypted_extensions
  wait_server_certificate --> wait_server_certificate_verify: certificate
  wait_server_certificate --> connected: finished (PSK/no-cert path)
  wait_server_certificate_verify --> wait_server_finished: certificate_verify
  wait_server_finished --> connected: finished
  connected --> connected: new_session_ticket / key_update
```

Server-side transition diagram (`δ_s`):

```mermaid
stateDiagram-v2
  [*] --> start
  start --> wait_client_certificate_or_finished: client_hello
  wait_client_certificate_or_finished --> wait_client_certificate_verify: certificate
  wait_client_certificate_or_finished --> connected: finished (no client cert)
  wait_client_certificate_verify --> wait_client_finished_after_cert: certificate_verify
  wait_client_finished_after_cert --> connected: finished
  connected --> connected: new_session_ticket / key_update
```

Terminal-state extension (engine-level):

```mermaid
flowchart LR
  connected -->|close_notify alert| closed
  connected -->|other alert| closing
  connected -->|EOF without close_notify| truncation["TruncationDetected (fail-closed)"]
  connected -->|EOF after close_notify| closed
```

Execution order in engine (`ingestHandshakePayload`):

1. `parseOne`
2. `validateHandshakeBody`
3. `transcript.update`
4. `classifyEvent`
5. `machine.onEvent(event)`

Validation runs before transition, so structurally invalid messages never enter the DFA.

Source: `src/tls13/state.zig`, `src/tls13/handshake.zig`, `src/tls13/session.zig`
